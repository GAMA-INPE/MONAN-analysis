...
The goal of all this is to obtain analysis pipelines that are reusable, robust, well organized and scalable. For that, an important step is integrating the created general functions and settings into our `src/` packages, which are installable, and will therefore be readily available for any future analysis script.
